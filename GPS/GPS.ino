...
void loop()
{
  ...
  while (ss.available() > 0) //while data is available
    {
     // Serial.println("data availabale");
    if (gps.encode(ss.read())) //read gps data
    {
      Serial.print("location isValid()");
 ...
 WiFiClient client = server.available(); // Check if a client has connected
  ...
  s += "width:50% >";
  s += " <form> <tr> <th><label for=";
  s += "lat_str2";
  s += ">Latitude  de point B:</label></th><br>";
  s += "<td ALIGN=CENTER ><input type='text'"; 
  s += "id='lat_str2'";
  s += "name='lat_str2'";
  s += "></td>/tr<><br>";
  s += "<tr><th><label for='lng_str2'";
  s += ">Longitude de point B:</label></th><br>";
  s += "<td ALIGN=CENTER ><input type='text'";
  s += "id='lng_str2'";
  s += "name='lng_str2'";
  s += "value='Submit'";
